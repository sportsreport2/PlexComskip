...
SAVE_ORIGINAL = config.getboolean('File Manipulation', 'save-original')
SAVE_ORIGINAL_LOCATION = config.get('File Manipulation', 'save-original-location')
DELETE_ORIGINAL = config.getboolean('File Manipulation', 'delete-original')
SAVE_SUBTITLES_SEPARATE_FILE = config.getboolean('File Manipulation', 'save-subtitles-separate-file')
NICE_LEVEL = config.get('Helper Apps', 'nice-level')
CONVERT = config.getboolean('File Manipulation', 'convert-original')
NEW_VIDEO_EXT = config.get('File Manipulation', 'convert-to')
PROCESS_FILE_TYPE = config.get('File Processing', 'process-file-type')
...
def done_processing():
  ...
  logging.info('Processing ended at: ' + str(time.time()))

def process(video_path):  
  logging.info('\n\n')
  logging.info('Processing started at: ' + str(time.time()))

  #Make sure passed in path exists before processing
  if not os.path.exists(video_path):
    logging.error('Passed in video path does not exist.')
    done_processing()
    sys.exit(0)
  ...
  try:
    if SAVE_ORIGINAL:
      if not os.path.isdir(SAVE_ORIGINAL_LOCATION):
        os.makedirs(SAVE_ORIGINAL_LOCATION)

      logging.info('Saving original file in save location: %s' % SAVE_ORIGINAL_LOCATION)
      shutil.copy(video_path, SAVE_ORIGINAL_LOCATION) 

  except Exception, e:
    logging.error('Something went wrong saving original file in save location: %s' % e)
    cleanup_and_exit(temp_dir, SAVE_ALWAYS or SAVE_FORENSICS)

  try:
    if COPY_ORIGINAL or SAVE_ALWAYS: 
      temp_video_path = os.path.join(temp_dir, orig_video_basename)
  ...
  edl_file = os.path.join(temp_dir, orig_video_name + '.edl')
  ...
  try:
    ...
    with open(segment_list_file_path, 'wb') as segment_list_file:
      for i, segment in enumerate(segments):
        ...
        segment_file_name = '%s%s' % (segment_name, orig_video_ext)
        if segment[1] == -1:
          duration_args = []
        else:
  ...
  try:
    cmd = NICE_ARGS + [FFMPEG_PATH, '-y', '-f', 'concat', '-i', segment_list_file_path, '-c', 'copy', os.path.join(temp_dir, orig_video_basename)]
    logging.info('[ffmpeg] Command: %s' % cmd)
    subprocess.call(cmd)
  ...
  try:
    ...
    output_size = os.path.getsize(os.path.join(temp_dir, orig_video_basename))
    if input_size and 1.01 > float(output_size) / float(input_size) > 0.99:
      ...
      if CONVERT and orig_video_ext != NEW_VIDEO_EXT:
        cmd = NICE_ARGS + [FFMPEG_PATH, '-i', os.path.join(temp_dir, orig_video_basename), '-i', os.path.join(temp_dir, subtitle_basename), '-c:s', 'copy', '-metadata:s:s:0', 'language=eng', os.path.join(temp_dir, new_video_basename)]
      ...
      if SAVE_SUBTITLES_SEPARATE_FILE:
...
#No input file, print message and exit
if len(sys.argv) < 2:
  print 'Usage: PlexComskip.py [input-file.mkv] [directory]'
  sys.exit(1) 

else:
  path = sys.argv[1]
  #If path is directory, process all files with extension in that directory
  if os.path.isdir(path):
    exclude = set(['.grab'])
    for root, directories, filenames in os.walk(path):
      directories[:] = [d for d in directories if d not in exclude]
      for filename in filenames:
        if filename.endswith(PROCESS_FILE_TYPE) and filename.find("Silicon Valley") == -1:
          process(os.path.join(root, filename))
  else:
    #Input file present. Process that input file.
    process(sys.argv[1])
